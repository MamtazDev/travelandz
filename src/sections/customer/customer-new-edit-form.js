  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_HOST_API}/customers/new`,
        data
      );
      console.log("customer create response", response.data);

      await new Promise((resolve) => setTimeout(resolve, 500));
      if (response.data.message == "Customer saved successfully") {
        reset();
      ...
      <Grid container spacing={3}>
        ...
        <Grid xs={12} md={8}>
          <Card sx={{ p: 3 }}>
            ...
            >
              <RHFTextField name="customerName" label="Full Customer Name" />
