...
export default function OverviewBookingView() {
  ...
  let companyId = companyContext.companyId;
  console.log('companyId: ', companyId)
  companyId = '64c8ff63916e9e9c2989891b'
  ...
  useEffect(() => {
    // Fetch work summary
    axios.get(`${process.env.REACT_APP_HOST_API}/tasks/summary/${companyId}`).then((response) => {
    console.log('response.data work: ', response.data)  
        const { totalProjectsThisYear, tasksSolvedThisYear, tasksOpenedThisYear } = response.data;
        setWorkSummary({
    ...
    axios.get(`${process.env.REACT_APP_HOST_API}/projects/summary/${companyId}`).then((response) => {
      console.log('response.data project: ', response.data)  
      setProjectsSummary(response.data);
    });
    // Fetch customers summary
    axios.get(`${process.env.REACT_APP_HOST_API}/customers/summary/${companyId}`).then((response) => {
      console.log('response.data customers: ', response.data)  
      setCustomersSummary(response.data);
    });
    // Fetch bookings summary
    axios.get(`${process.env.REACT_APP_HOST_API}/bookings/summary/${companyId}`).then((response) => {
  ...
  return (
    <Container maxWidth={settings.themeStretch ? false : 'xl'}>
      <Grid container spacing={SPACING} disableEqualOverflow>
        <Grid xs={12} md={4}>
          <BookingWidgetSummary
            ...
            total='58'//{workSummary.totalProjectsThisYear} // Use the value from workSummary
        ...
        <BookingWidgetSummary
          title="Tasks Solved"
          total='104'//{workSummary.tasksSolvedThisYear} // Use the value from workSummary
      ...
      <Grid xs={12} md={4}>
        <BookingWidgetSummary
          title="Tasks Opened"
          total='32'//{workSummary.tasksOpenedThisYear} // Use the value from workSummary
          ...
          <Grid container xs={12} md={8}>
            ...
            <Grid xs={12}>
              <BookingCheckInWidgets
                chart={{
                  series: [
                    { label: 'Sold', percent: 72, total: 138566 },
                    { label: 'Pending for payment', percent: 64, total: 118472 },
                  ],
                }}
              />
            </Grid>
